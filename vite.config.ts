...
export default defineConfig(({ command }) => ({ base: command === 'build' ? (process.env.VITE_BASE_PATH ?? '/digital-garden/') : '/', plugins: [react(), tailwindcss(), ...(command === 'serve' ? [developerPlugin()] : [])] }))
